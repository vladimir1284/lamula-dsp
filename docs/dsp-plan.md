...
Those five are the headline set. The **full** algorithm set — the one that covers every knob the frozen `DSP↔RCP` contract already exposes — additionally comprises the I/Q signal simulator (Zrnić, 1975), noise estimation and the four censoring thresholds, the SQI/CCOR/SIG quality indices, the spectral estimator, Tx-burst phase correction and the AFC loop, range processing and the split/batch/Doppler cut modes, narrowband-RFI filtering, clutter-map generation and application, staggered-PRT, the polarimetric covariance estimators (ZDR/ρHV/ΦDP/LDR), KDP estimation, polarimetric calibration, range (multi-trip) dealiasing and the IF spectrum analyser. Each has a design page under `docs/algorithms/`, and the **[algorithm study & implementation roadmap](algorithms/roadmap.md)** sequences them against the phases in §8.2, fixes the common working method (Python oracle → Rust → numerical cross-check) and the common acceptance criterion (measured bias and standard deviation against the theoretical estimator variance, not a single-value tolerance).

Two hardware axes run through the whole set and neither is fixed by this project: the transmitter is either a **magnetron** — free-running, random per-pulse phase, drifting frequency, hence mandatory coherent-on-receive phase correction and an AFC loop — or a coherent **klystron/TWT/solid-state** amplifier; and polarimetry is either **absent** (single channel), **simultaneous** (STAR: ZDR/ΦDP/ρHV, no LDR, cross-coupling bias) or **alternating** (LDR available, half the effective PRF, different ρHV/ΦDP estimators). Consequently the moment set and the mode set a given installation produces are a **runtime capability**, declared through `capabilities`/`capability_flags` and rejected cleanly via `moment_unsupported`/`dealias_unsupported`/`estimator_unsupported` — not a compile-time constant. One scheduling consequence is already reflected in the roadmap: on a magnetron installation the Tx-burst phase correction is a hard prerequisite of pulse-pair estimation and moves from Phase 2 to Phase 1.

These map directly onto the Moment Estimator, Clutter Filter, Polarimetry Engine, Dealiasing, IF Spectrum Analyzer and Control/Config Plane components in §4.4.
