...
* **DRX/RSP Control & Calibration.** Grouped into TX/RX adjustment (transmit power setpoint, receiver gain/attenuation stages) and calibration sub-panels (see below), plus a DRX process-monitor view showing the moment-ingestion pipeline's health (link up/down, ray rate, dropped-ray counters) — this is the operator-facing counterpart of the DSP/DRX Moment Ingestion component in §4.4.
* **Data Views (PPI / RHI / ASCOPE).** Each view shares one color-management subsystem (a 256-level color table per data type, editable via a color-composer panel) rather than three independent renderers. PPI supports pan/zoom and freeze; RHI shares the azimuth cursor with the active PPI; ASCOPE renders the raw I/Q or moment profile along the current beam for calibration and diagnostic use. Multi-data-type switching (UZ/CZ/V/W/ZDR/…) is a single control shared across all three views, not per-view state.
